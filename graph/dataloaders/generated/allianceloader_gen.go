...
type AllianceLoaderConfig struct {
	...
	Fetch func(keys []int) ([][]*monocle.Alliance, []error)
...
type AllianceLoader struct {
	...
	cache map[int][]*monocle.Alliance
...
func (l *AllianceLoader) Load(key int) ([]*monocle.Alliance, error) {
...
func (l *AllianceLoader) LoadThunk(key int) func() ([]*monocle.Alliance, error) {
	l.mu.Lock()
...
func (l *AllianceLoader) LoadAll(keys []int) ([][]*monocle.Alliance, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *AllianceLoader) LoadAllThunk(keys []int) func() ([][]*monocle.Alliance, []error) {
	results := make([]func() ([]*monocle.Alliance, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *AllianceLoader) Prime(key int, value []*monocle.Alliance) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*monocle.Alliance, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *AllianceLoader) Clear(key int) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *AllianceLoader) unsafeSet(key int, value []*monocle.Alliance) {
	if l.cache == nil {
		l.cache = map[int][]*monocle.Alliance{}
	}
	l.cache[key] = value
}
...
func (b *allianceLoaderBatch) keyIndex(l *AllianceLoader, key int) int {
