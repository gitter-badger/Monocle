...
// NewCorporationMembersLoader creates a new CorporationMembersLoader given a fetch, wait, and maxBatch
func NewCorporationMembersLoader(config CorporationMembersLoaderConfig) *CorporationMembersLoader {
	return &CorporationMembersLoader{
		fetch:    config.Fetch,
		wait:     config.Wait,
...
type CorporationMembersLoader struct {
	...
	batch *corporationMembersLoaderBatch
...
func (l *CorporationMembersLoader) LoadThunk(key uint) func() ([]*monocle.Character, error) {
	...
	if l.batch == nil {
		l.batch = &corporationMembersLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *CorporationMembersLoader) Prime(key uint, value []*monocle.Character) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*monocle.Character, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (b *corporationMembersLoaderBatch) startTimer(l *CorporationMembersLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *corporationMembersLoaderBatch) end(l *CorporationMembersLoader) {
