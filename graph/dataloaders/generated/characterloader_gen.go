...
type CharacterLoaderConfig struct {
	...
	Fetch func(keys []uint64) ([]*monocle.Character, []error)
...
type CharacterLoader struct {
	...
	cache map[uint64]*monocle.Character
...
func (l *CharacterLoader) Load(key uint64) (*monocle.Character, error) {
...
func (l *CharacterLoader) LoadThunk(key uint64) func() (*monocle.Character, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*monocle.Character, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *CharacterLoader) LoadAll(keys []uint64) ([]*monocle.Character, []error) {
	...
	characters := make([]*monocle.Character, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *CharacterLoader) LoadAllThunk(keys []uint64) func() ([]*monocle.Character, []error) {
	results := make([]func() (*monocle.Character, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *CharacterLoader) Prime(key uint64, value *monocle.Character) bool {
	l.mu.Lock()
	var found bool
...
func (l *CharacterLoader) unsafeSet(key uint64, value *monocle.Character) {
	if l.cache == nil {
		l.cache = map[uint64]*monocle.Character{}
	}
	l.cache[key] = value
}
...
func (b *characterLoaderBatch) keyIndex(l *CharacterLoader, key uint64) int {
